...
const DetailModal = ({ isOpen, onClose, title, data, fieldOrder = [], fieldLabels = {} }) => {
  ...
  // Function untuk format nilai berdasarkan tipe data
  const formatValue = (key, value) => {
    // Handle array (untuk penyakit_warga dan field array lainnya)
    ...
    <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-center p-4 animate-fade-in">
      <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-lg transform transition-all animate-fade-in-up">
        ...
        <div className="mt-6 text-right">
          <button onClick={onClose} className="bg-slate-200 text-slate-700 px-4 py-2 rounded-lg hover:bg-slate-300 transition duration-200">
            Tutup
          </button>
        </div>
      </div>
